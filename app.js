...
guessRows.forEach((row, index) => {
    ...
    row.setAttribute('id', 'guessRow-' + index)
...
const submitGuess = () => {
    ...
    if (currentTile > 3) {
        if(!dictionary.includes(guess)){
            showAlert("Not in word list", false)
            return
        }
        else {
            flipTile()
    ...
    else showAlert("Not Enough Letters", false)
}
...
const flipTile = () => {
    ...
    rowTiles.forEach((tile, index) => {
        setTimeout(() => {
            ...
            coloring(guess[index].letter, guess[index].color)
        }, (500 * index)/2)
    })
}
